...
- **Node.js** (`npx`) — нужен для запуска MCP-сервера `context7`.
...
Скрипт регистрирует хуки в локальной директории `.claude/` относительно проекта, на котором будет работать Claude Code, и пытается зарегистрировать MCP-серверы (см. ниже). Для `context7` после установки нужно вручную подставить API-ключ.
