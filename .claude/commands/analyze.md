...
Следующий шаг после approve — `/dev_plan`.
...
2. Иди по секциям и задавай вопросы заказчику через `AskUserQuestion`
   ...
   **не выводится однозначно** из задачи; по каждому вопросу предлагай
   ...
   библиотеки, версии) — это зона `/dev_plan`; при таких вопросах
...
- **`yes`** (или любая утвердительная форма) — выведи финальное:
  ...
  **Next step**: запусти `/dev_plan` с этим `increment.md` как
  основным контекстом.
  ```
...
- Не вызывай `/dev_plan` сам. Это следующий шаг пользователя.
