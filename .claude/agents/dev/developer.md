...
- Прочитай **все назначенные задачи из промпта** (`ASSIGNED_TASKS` — их полные
  описания передаёт оркестратор; у тебя нет Task-инструментов). Пойми их общий
  контекст и порядок (зависимости `Depends On`).
...
- Выполняй назначенные задачи **по порядку зависимостей**, одну за другой, в
  своём контексте. **Статусы в общем task-леджере ведёт оркестратор** (у тебя
  нет Task-инструментов) — не вызывай TaskUpdate/TaskGet, просто перечисли
  сделанное в Report.
...
- Хуки прогоняют линтеры/форматтеры на каждый Write/Edit. **Форматирование
  (spotless/prettier/ruff) применяется автоматически — НЕ подгоняй пробелы и
  отступы руками.** Если хук всё же блокирует — это содержательная проблема
  (компиляция, реальный lint, безопасность): исправь суть и повтори, не обходи.
...
4. По каждой задаче: напиши продуктовый код → дождись авто-валидации (PostToolUse
   линтеры; форматирование применяется само); при содержательном блоке — правь суть.
