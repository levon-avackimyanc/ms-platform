...
- Хуки прогоняют линтеры/форматтеры на каждый Write/Edit в **authoring-режиме**
  (`--authoring`): форматирование (`spotless`) и статанализ (`pmd`) гейтят, а
  **компиляция и покрытие (`maven_compile`/`jacoco`) отложены до `/test_run`** —
  поэтому ссылки на ещё не собранный код сервиса (параллельная разработка) **не
  блокируют** авторинг. **Формат применяется автоматически — не подгоняй пробелы
  руками.** При содержательном блоке (`pmd`/формат) — правь суть и повтори; пробел
  в контракте сервиса фиксируй в Report, тест пиши на ожидаемое поведение.
