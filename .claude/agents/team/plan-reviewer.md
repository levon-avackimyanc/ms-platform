...
| 8 | **Cost Appropriateness** — Are models and agent types used proportionally to task complexity? **Dev-scope model policy is intentional: `developer` runs on Sonnet (bulk code-writing), `code-reviewer` runs on Opus (critical diff-review gate) — do NOT flag this pairing as a mismatch.** | Opus for complex reasoning, Sonnet/Haiku for routine, scripts for deterministic; the deliberate Dev-scope `developer`=Sonnet / `code-reviewer`=Opus split is correct | Opus for trivial tasks, or Haiku for complex reasoning | Minor optimization possible |
