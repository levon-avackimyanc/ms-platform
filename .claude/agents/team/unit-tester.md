...
- Для web-DTO/сериализуемых ответов проверяй **сериализованный JSON** (форма тела,
  `null`-поля), а не только аксессоры record'а — иначе баги сериализации проходят
  мимо юнитов.
...
3. По каждой задаче: найди символы → напиши unit-тесты → дождись авто-валидации
   (форматирование применяется само); при содержательном блоке — правь суть.
